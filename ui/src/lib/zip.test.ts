...
function createZipArchiveWithDirectoryEntries(rootPath: string) {
  ...
  const entries = [
    ...
    { path: `${rootPath}/agents/beep/`, body: new Uint8Array(0), compressionMethod: 0 },
    { path: `${rootPath}/COMPANY.md`, body: encoder.encode("# Company\n"), compressionMethod: 8 },
    { path: `${rootPath}/agents/beep/AGENTS.md`, body: encoder.encode("# BEEP\n"), compressionMethod: 8 },
  ].map((entry) => ({
    ...entry,
...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("paperclip-demo/agents/beep/AGENTS.md");
    expect(readString(archive, 30 + firstNameLength, firstBodyLength)).toBe("# BEEP\n");
